...
public class AuthHelperTable {
    ...
    public static String getAccessToken() throws OApiException {
        ...
        JSONObject accessTokenValue = (JSONObject) FileUtils.getValue("accesstoken", EnvProjectTable.APP_KEY);
        String accToken = "";
        ...
        if (accessTokenValue == null || curTime - accessTokenValue.getLong("begin_time") >= cacheTime) {
            try {
                ...
                accToken = corpConnectionService.getCorpToken(EnvProjectTable.APP_KEY, EnvProjectTable.APP_SECRET);
                // save accessToken
                JSONObject jsonAccess = new JSONObject();
                jsontemp.clear();
                jsontemp.put("access_token", accToken);
                jsontemp.put("begin_time", curTime);
                jsonAccess.put(EnvProjectTable.APP_KEY, jsontemp);
                //真实项目中最好保存到数据库中
    ...
    public static String getJsapiTicket(String accessToken) throws OApiException {
        JSONObject jsTicketValue = (JSONObject) FileUtils.getValue("jsticket", EnvProjectTable.APP_KEY);
        long curTime = System.currentTimeMillis();
        String jsTicket = "";
        ...
        if (jsTicketValue == null || curTime -
            ...
            try {
                ...
                jsontemp.put("begin_time", curTime);
                jsonTicket.put(EnvProjectTable.APP_KEY, jsontemp);
    ...
    public static String getConfig(HttpServletRequest request) {
        ...
        try {
            accessToken = AuthHelperTable.getAccessToken();

            ticket = AuthHelperTable.getJsapiTicket(accessToken);
            signature = AuthHelperTable.sign(ticket, nonceStr, timeStamp, signedUrl);
        ...
        configValue.put("corpId", EnvProjectTable.CORP_ID);
        configValue.put("agentId", EnvProjectTable.AGENT_ID);
    ...
    public static String getSsoToken() throws OApiException {
        String url = "https://oapi.dingtalk.com/sso/gettoken?corpid=" + EnvProjectTable.CORP_ID + "&corpsecret=" + EnvProjectTable.SSO_Secret;
