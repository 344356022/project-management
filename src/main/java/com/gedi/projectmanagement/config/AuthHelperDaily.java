...
public class AuthHelperDaily {
    ...
    public static String getAccessToken() throws OApiException {
        ...
        JSONObject accessTokenValue = (JSONObject) FileUtils.getValue("accesstoken", EnvDaily.APP_KEY);
        String accToken = "";
        ...
        if (accessTokenValue == null || curTime - accessTokenValue.getLong("begin_time") >= cacheTime) {
            try {
                ...
                accToken = corpConnectionService.getCorpToken(EnvDaily.APP_KEY, EnvDaily.APP_SECRET);
                // save accessToken
                JSONObject jsonAccess = new JSONObject();
                jsontemp.clear();
                jsontemp.put("access_token", accToken);
                jsontemp.put("begin_time", curTime);
                jsonAccess.put(EnvDaily.APP_KEY, jsontemp);
                //真实项目中最好保存到数据库中
    ...
    public static String getJsapiTicket(String accessToken) throws OApiException {
        JSONObject jsTicketValue = (JSONObject) FileUtils.getValue("jsticket", EnvDaily.APP_KEY);
        long curTime = System.currentTimeMillis();
        String jsTicket = "";
        ...
        if (jsTicketValue == null || curTime -
            ...
            try {
                ...
                jsontemp.put("begin_time", curTime);
                jsonTicket.put(EnvDaily.APP_KEY, jsontemp);
    ...
    public static String getConfig(HttpServletRequest request) {
        ...
        try {
            accessToken = AuthHelperDaily.getAccessToken();

            ticket = AuthHelperDaily.getJsapiTicket(accessToken);
            signature = AuthHelperDaily.sign(ticket, nonceStr, timeStamp, signedUrl);
        ...
        configValue.put("corpId", EnvDaily.CORP_ID);
        configValue.put("agentId", EnvDaily.AGENT_ID);
    ...
    public static String getSsoToken() throws OApiException {
        String url = "https://oapi.dingtalk.com/sso/gettoken?corpid=" + EnvDaily.CORP_ID + "&corpsecret=" + EnvDaily.SSO_Secret;
