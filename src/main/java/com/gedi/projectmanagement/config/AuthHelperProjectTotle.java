...
import com.gedi.projectmanagement.util.FileUtils;
import com.gedi.projectmanagement.util.HttpHelper;
import com.gedi.projectmanagement.util.OApiException;
 ...
 */
public class AuthHelperProjectTotle {
    ...
    public static String getAccessToken() throws OApiException {
        ...
        JSONObject accessTokenValue = (JSONObject) FileUtils.getValue("accesstoken", EnvProjectTotle.APP_KEY);
        String accToken = "";
        ...
        if (accessTokenValue == null || curTime - accessTokenValue.getLong("begin_time") >= cacheTime) {
            try {
                ...
                accToken = corpConnectionService.getCorpToken(EnvProjectTotle.APP_KEY, EnvProjectTotle.APP_SECRET);
                // save accessToken
                JSONObject jsonAccess = new JSONObject();
                jsontemp.clear();
                jsontemp.put("access_token", accToken);
                jsontemp.put("begin_time", curTime);
                jsonAccess.put(EnvProjectTotle.APP_KEY, jsontemp);
                //真实项目中最好保存到数据库中
    ...
    public static String getJsapiTicket(String accessToken) throws OApiException {
        JSONObject jsTicketValue = (JSONObject) FileUtils.getValue("jsticket", EnvProjectTotle.APP_KEY);
        long curTime = System.currentTimeMillis();
        String jsTicket = "";
        ...
        if (jsTicketValue == null || curTime -
            ...
            try {
                ...
                jsontemp.put("begin_time", curTime);
                jsonTicket.put(EnvProjectTotle.APP_KEY, jsontemp);
    ...
    public static String getConfig(HttpServletRequest request) {
        ...
        try {
            accessToken = AuthHelperProjectTotle.getAccessToken();

            ticket = AuthHelperProjectTotle.getJsapiTicket(accessToken);
            signature = AuthHelperProjectTotle.sign(ticket, nonceStr, timeStamp, signedUrl);
        ...
        configValue.put("corpId", EnvProjectTotle.CORP_ID);
        configValue.put("agentId", EnvProjectTotle.AGENT_ID);
    ...
    public static String getSsoToken() throws OApiException {
        String url = "https://oapi.dingtalk.com/sso/gettoken?corpid=" + EnvProjectTotle.CORP_ID + "&corpsecret=" + EnvProjectTotle.SSO_Secret;
